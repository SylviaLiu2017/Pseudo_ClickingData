...
rowvec SampleForOneUserClicks(rowvec clicking,rowvec data_curr, rowvec rho_curr, double alpha, double sigma);
...
List PseudoForClicks(int n_samples,mat clicking_data, mat data_init, double alpha, double sigma);
...
List PseudoForClicks(int n_samples,mat clicking_data, mat data_init, double alpha, double sigma){
  ...
  for(int i=0; i<n_samples; i++){
    ...
    rho_curr= SampleRho(data_curr, alpha,sigma);
    rhoMat.row(i) = rho_curr;
    for(int j =0; j<N; j++){
      data_curr.row(j) = SampleForOneUserClicks(clicking_data.row(j), rho_curr, alpha,sigma);
    }
